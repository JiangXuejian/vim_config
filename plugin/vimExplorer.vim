...
if (has("win32") || has("win95") || has("win64") || has("win16") || has("dos32"))
    ...
    if exists("g:VEConf_externalExplorer")
        ...
        let VEConf.externalExplorer = "open" 
"replace nautilus here"
    endif
endif
...
function! s:VEFilePanel.itemClicked(line)
    ...
    if path == ''
        "call g:VEPlatform.system("open " . g:VEPlatform.escape(a:path))
        return
    endif
    if isdirectory(path)
        call VE_GotoPath(path)
...
<
"test1" is the key. VimExplorer will bind the hotkey and corresponding actions automatically.
>
...
>
    ...
    function! VEConf_multiFileActions['test3'](fileList)
        for i in a:fileList
            call g:VEPlatform.start("open" . i)
        endfor
    endfunction
<
